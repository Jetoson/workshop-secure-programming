...
# Exercise 02 - Command Injection Fix Verifier
...
echo "============================================================"
echo " Exercise 02: Command Injection - Verify Fix"
echo "============================================================"
echo ""

echo ""
echo "[TEST 1] Normal input should still work..."
OUTPUT=$(echo "127.0.0.1" | python3 vuln_code.py 2>&1 || true)
if echo "$OUTPUT" | grep -qiE "ping|ttl|transmitted|1 received|alive|respond"; then
...
OUTPUT=$(echo "127.0.0.1; echo INJECTION_SUCCEEDED" | python3 vuln_code.py 2>&1 || true)
...
OUTPUT=$(echo "127.0.0.1 && id" | python3 vuln_code.py 2>&1 || true)
